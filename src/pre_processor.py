    ...
    #     FILE.write(FILE.readline())
        ...
        if sys.argv[1] in "-help":
            ...
            if os.path.exists(os.getcwd() + "/" + sys.argv[1]):
                ...
                print(
                    f"Directorio de volcado del programa creado en: {OUTPUTDIR}")
...
class Token:
    ...
    def print(self):
        print(
            f"{self.code},{self.att} line: {self.line}, cols: {self.startCol} -> {self.endCol}")
...
class Lexer:
    ...
    def genToken(self, code: str, attribute=None) -> Token:
        ...
        generated_token = Token(
            code, self.line, self.startCol, self.col, attribute)
        global TOKENLIST
...
class Syntactic:
            ...
            if not Sp or isinstance(Sp.tipo, list):  # es una llamada a funcion
                ...
                if not var_tabla:
                    if Sp.tipo == "int":
                        ...
                        self.error(
                            "TypeError", f"Solo se pueden hacer asignaciones sin inicializacion cuando la asignacion es de tipo int, en este caso es de tipo {Sp.tipo}")
                    ...
                    self.error(
                        "TypeError", f"Tipo de la variable: \"{var_tabla.tipo}\" no coincide con el tipo de la asignación: \"{Sp.tipo}\"")
    ...
    def O1(self, prev=None) -> ProductionObject:
        if self.equipara("or", 36):
            ...
            if N.tipo != "boolean":
                self.error(
                    "WrongDataTypeError", f"Operador || solo acepta datos lógicos, tipo dado {N.tipo}")
            return self.O1()
        elif self.equipara("and", 37):
            N = self.N()
            if N.tipo != "boolean":
                self.error(
                    "OperandTypeError", f"Operador && solo acepta datos lógicos, tipo dado {N.tipo}")
            return self.O1()
    ...
    def O2(self, prev=None) -> ProductionObject:
        if self.equipara("equals", 39):
            ...
            if Z.tipo != "int":
                self.error(
                    "OperandTypeError", f"Operador == solo acepta datos de tipo entero, tipo dado {Z.tipo}")
            return self.O2()
    ...
    def O3(self, prev=None) -> ProductionObject:
        if self.equipara("mas", 42):
            ...
            if R.tipo != "int":
                self.error(
                    "OperandTypeError", f"Operador + solo acepta datos enteros, tipo dado {R.tipo}")
            else:
                return self.O3()
